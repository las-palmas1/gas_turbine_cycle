...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		\[
			c_{pг\ ис} (T_{тк}^*, \alpha) = << source.c_p_comb_products_true | round(2) >>\ Дж/(кг \cdot К)
		\]
		...
		\[
			c_{pг} (T_{см}^*, \alpha_{см}) = << source.mixture.c_p | round(3) >>\ Дж/(кг \cdot К)
		\]
		...
		\begin{gather*}
			...
			= \frac{
                        << source.c_p_comb_products_true | round(2) >> \cdot << source.T_stag_in | round(2) >> \cdot
						<< source.g_in | round(3) >> +
						<< source.c_p_air_true | round(2) >> \cdot << source.T_return | round(2) >> \cdot
						<< source.g_return | round(3) >>
                    }{
                        << source.mixture.c_p | round(3) >> \cdot  << source.g_out | round(3) >>
                    } =
